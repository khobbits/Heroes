...
public class SkillSoulBond extends TargettedSkill {
    ...
    public boolean use(Hero hero, LivingEntity target, String[] args) {
        ...
        if (target instanceof Player) {
            ...
            plugin.getHeroManager().addCreatureEffect((Creature) target, sbEffect);
        }
        ...
        public void onSkillDamage(SkillDamageEvent event) {
            ...
            if (event.getEntity() instanceof Player) {
                ...
                if (tHero.hasEffect("SoulBonded") && !tHero.hasEffect("SoulBond")) {
                    ...
                    // Distance check
                    int radiusSquared = (int) Math.pow(getSetting(hero.getHeroClass(), Setting.RADIUS.node(), 25), 2);
                    if (applier.getLocation().distanceSquared(event.getEntity().getLocation()) > radiusSquared)
                        return;

                    // Split the damage
                    int splitDamage = (int) (event.getDamage() * getSetting(hero.getHeroClass(), "damage-multiplier", .5));
                ...
                if (!plugin.getHeroManager().creatureHasEffect((Creature) event.getEntity(), "SoulBonded"))
                    return;

                Player applier = ((SoulBondedEffect) plugin.getHeroManager().getCreatureEffect((Creature) event.getEntity(), "SoulBonded")).getApplier();
                Hero hero = plugin.getHeroManager().getHero(applier);
    ...
    public class SoulBondEffect extends ExpirableEffect {
        ...
        public void remove(Hero hero) {
            ...
            if (target instanceof Player) {
                ...
                plugin.getHeroManager().removeCreatureEffect((Creature) target, bondEffect);
            }
